inherited fmCadCliente: TfmCadCliente
  Caption = ''
  ClientHeight = 582
  ClientWidth = 940
  ExplicitWidth = 956
  ExplicitHeight = 620
  PixelsPerInch = 96
  TextHeight = 13
  inherited ToolBar1: TToolBar
    Width = 940
    ExplicitWidth = 940
    inherited btEditar: TToolButton
      OnClick = btEditarClick
    end
    inherited btExcluir: TToolButton
      OnClick = btExcluirClick
    end
  end
  inherited pnPesquisa: TPanel
    Width = 940
    ExplicitWidth = 940
    inherited btPesquisar: TSpeedButton
      OnClick = btPesquisarClick
    end
  end
  inherited PageControl1: TPageControl
    Width = 940
    Height = 495
    ActivePage = tbCadastro
    ExplicitWidth = 940
    ExplicitHeight = 495
    inherited tbDados: TTabSheet
      ExplicitLeft = 4
      ExplicitTop = 26
      ExplicitWidth = 932
      ExplicitHeight = 465
      inherited pnLegenda: TPanel
        Top = 436
        Width = 932
        ExplicitTop = 436
        ExplicitWidth = 932
      end
      inherited DBGrid1: TDBGrid
        Width = 932
        Height = 436
        DataSource = dsG
        OnDrawColumnCell = DBGrid1DrawColumnCell
        Columns = <
          item
            Expanded = False
            FieldName = 'ID'
            Title.Caption = 'C'#243'digo'
            Width = 64
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME'
            Title.Caption = 'Nome'
            Width = 308
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CADASTRO'
            Title.Caption = 'Cadastro'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CPF'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RG'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DATA_NASCIMENTO'
            Title.Caption = 'Nascimento'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TELEFONE'
            Title.Caption = 'Telefone'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CELULAR'
            Title.Caption = 'Celular'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TEL_RECADO'
            Title.Caption = 'Tel. Recado'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ATIVO'
            Title.Caption = 'Ativo'
            Visible = True
          end>
      end
    end
    inherited tbCadastro: TTabSheet
      ExplicitLeft = 4
      ExplicitTop = 26
      ExplicitWidth = 932
      ExplicitHeight = 465
      object Label3: TLabel
        Left = 5
        Top = 3
        Width = 38
        Height = 15
        Caption = 'C'#243'digo'
        FocusControl = DBEdit1
      end
      object Label4: TLabel
        Left = 80
        Top = 3
        Width = 48
        Height = 15
        Caption = 'Cadastro'
        FocusControl = DBEdit2
      end
      object Label5: TLabel
        Left = 5
        Top = 45
        Width = 34
        Height = 15
        Caption = 'Nome'
        FocusControl = DBEdit3
      end
      object Label6: TLabel
        Left = 530
        Top = 44
        Width = 20
        Height = 15
        Caption = 'CPF'
        FocusControl = DBEdit4
      end
      object Label7: TLabel
        Left = 645
        Top = 44
        Width = 17
        Height = 15
        Caption = 'RG'
        FocusControl = DBEdit5
      end
      object Label8: TLabel
        Left = 443
        Top = 44
        Width = 66
        Height = 15
        Caption = 'Nascimento'
        FocusControl = DBEdit6
      end
      object Label9: TLabel
        Left = 3
        Top = 87
        Width = 50
        Height = 15
        Caption = 'Telefone'
        FocusControl = DBEdit7
      end
      object Label10: TLabel
        Left = 106
        Top = 88
        Width = 38
        Height = 15
        Caption = 'Celular'
        FocusControl = DBEdit8
      end
      object Label11: TLabel
        Left = 217
        Top = 87
        Width = 64
        Height = 15
        Caption = 'Tel. Recado'
        FocusControl = DBEdit9
      end
      object Label12: TLabel
        Left = 174
        Top = 3
        Width = 79
        Height = 15
        Caption = 'Limite Cr'#233'dito'
        FocusControl = DBEdit10
      end
      object Label13: TLabel
        Left = 5
        Top = 129
        Width = 34
        Height = 15
        Caption = 'E-mail'
        FocusControl = DBEdit11
      end
      object Label14: TLabel
        Left = 444
        Top = 129
        Width = 44
        Height = 15
        Caption = 'E-mail 2'
        FocusControl = DBEdit12
      end
      object Label19: TLabel
        Left = 5
        Top = 174
        Width = 24
        Height = 15
        Caption = 'M'#227'e'
        FocusControl = DBEdit13
      end
      object Label20: TLabel
        Left = 444
        Top = 174
        Width = 16
        Height = 15
        Caption = 'Pai'
        FocusControl = DBEdit14
      end
      object DBEdit1: TDBEdit
        Left = 5
        Top = 19
        Width = 73
        Height = 23
        DataField = 'ID'
        DataSource = dsG
        TabOrder = 0
      end
      object DBEdit2: TDBEdit
        Left = 80
        Top = 19
        Width = 92
        Height = 23
        DataField = 'CADASTRO'
        DataSource = dsG
        TabOrder = 1
      end
      object DBEdit3: TDBEdit
        Left = 5
        Top = 60
        Width = 436
        Height = 23
        DataField = 'NOME'
        DataSource = dsG
        TabOrder = 5
        OnExit = DBEdit3Exit
      end
      object DBEdit4: TDBEdit
        Left = 530
        Top = 60
        Width = 113
        Height = 23
        DataField = 'CPF'
        DataSource = dsG
        TabOrder = 7
        OnExit = DBEdit4Exit
      end
      object DBEdit5: TDBEdit
        Left = 645
        Top = 60
        Width = 129
        Height = 23
        DataField = 'RG'
        DataSource = dsG
        TabOrder = 8
      end
      object DBEdit6: TDBEdit
        Left = 443
        Top = 60
        Width = 85
        Height = 23
        DataField = 'DATA_NASCIMENTO'
        DataSource = dsG
        TabOrder = 6
      end
      object DBEdit7: TDBEdit
        Left = 3
        Top = 103
        Width = 101
        Height = 23
        DataField = 'TELEFONE'
        DataSource = dsG
        TabOrder = 9
      end
      object DBEdit8: TDBEdit
        Left = 106
        Top = 104
        Width = 108
        Height = 23
        DataField = 'CELULAR'
        DataSource = dsG
        TabOrder = 10
      end
      object DBEdit9: TDBEdit
        Left = 217
        Top = 103
        Width = 94
        Height = 23
        DataField = 'TEL_RECADO'
        DataSource = dsG
        TabOrder = 11
      end
      object DBEdit10: TDBEdit
        Left = 174
        Top = 19
        Width = 107
        Height = 23
        DataField = 'LIMITECREDITO'
        DataSource = dsG
        TabOrder = 2
      end
      object DBEdit11: TDBEdit
        Left = 5
        Top = 144
        Width = 436
        Height = 23
        DataField = 'EMAIL'
        DataSource = dsG
        TabOrder = 12
      end
      object DBEdit12: TDBEdit
        Left = 444
        Top = 144
        Width = 436
        Height = 23
        DataField = 'EMAIL2'
        DataSource = dsG
        TabOrder = 13
      end
      object DBEdit13: TDBEdit
        Left = 5
        Top = 189
        Width = 436
        Height = 23
        DataField = 'NOME_MAE'
        DataSource = dsG
        TabOrder = 14
      end
      object DBEdit14: TDBEdit
        Left = 444
        Top = 189
        Width = 436
        Height = 23
        DataField = 'NOME_PAI'
        DataSource = dsG
        TabOrder = 15
      end
      object DBCheckBox1: TDBCheckBox
        Left = 287
        Top = 22
        Width = 72
        Height = 17
        Caption = ' Ativo'
        DataField = 'ATIVO'
        DataSource = dsG
        TabOrder = 3
        ValueChecked = 'S'
        ValueUnchecked = 'N'
      end
      object DBCheckBox2: TDBCheckBox
        Left = 358
        Top = 22
        Width = 72
        Height = 17
        Caption = ' Fisica'
        DataField = 'TIPO'
        TabOrder = 4
        ValueChecked = 'F'
        ValueUnchecked = 'J'
        Visible = False
      end
      object Panel1: TPanel
        Left = 3
        Top = 218
        Width = 877
        Height = 251
        BevelOuter = bvNone
        TabOrder = 16
        object PageControl2: TPageControl
          Left = 0
          Top = 0
          Width = 877
          Height = 251
          ActivePage = TabSheet1
          Align = alClient
          TabOrder = 0
          object TabSheet1: TTabSheet
            Caption = 'Endere'#231'o'
            object Label17: TLabel
              Left = 2
              Top = 1
              Width = 38
              Height = 15
              Caption = 'C'#243'digo'
              FocusControl = DBCod
            end
            object Label18: TLabel
              Left = 2
              Top = 39
              Width = 52
              Height = 15
              Caption = 'Endere'#231'o'
              FocusControl = dbEndereco
            end
            object Label21: TLabel
              Left = 312
              Top = 39
              Width = 14
              Height = 15
              Caption = 'N'#186
              FocusControl = dbNumero
            end
            object Label22: TLabel
              Left = 89
              Top = -1
              Width = 20
              Height = 15
              Caption = 'CEP'
              FocusControl = DBEdit18
            end
            object Label23: TLabel
              Left = 366
              Top = 39
              Width = 79
              Height = 15
              Caption = 'Complemento'
              FocusControl = dbLogradouro
            end
            object Label24: TLabel
              Left = 2
              Top = 80
              Width = 34
              Height = 15
              Caption = 'Bairro'
              FocusControl = dbBairro
            end
            object Label25: TLabel
              Left = 343
              Top = 80
              Width = 37
              Height = 15
              Caption = 'Cidade'
              FocusControl = dbCidade
            end
            object btSalvarEnd: TSpeedButton
              Left = 700
              Top = 51
              Width = 26
              Height = 29
              Glyph.Data = {
                
                EFE3FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
              OnClick = btSalvarEndClick
            end
            object btEditarEnd: TSpeedButton
              Left = 724
              Top = 51
              Width = 27
              Height = 29
              Glyph.Data = {
                
                FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
              OnClick = btEditarEndClick
            end
            object btExcluirEnd: TSpeedButton
              Left = 748
              Top = 51
              Width = 27
              Height = 29
              Glyph.Data = {
                
                FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
              OnClick = btExcluirEndClick
            end
            object Label26: TLabel
              Left = 676
              Top = 80
              Width = 15
              Height = 15
              Caption = 'UF'
              Font.Charset = ANSI_CHARSET
              Font.Color = clWindowText
              Font.Height = -12
              Font.Name = 'Segoe UI'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object btNovoEnd: TSpeedButton
              Left = 676
              Top = 51
              Width = 26
              Height = 29
              Glyph.Data = {
                
                F6F0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
              OnClick = btNovoEndClick
            end
            object DBCod: TDBEdit
              Left = 2
              Top = 15
              Width = 82
              Height = 23
              DataField = 'ID'
              DataSource = dsEndereco
              TabOrder = 0
            end
            object dbEndereco: TDBEdit
              Left = 3
              Top = 55
              Width = 307
              Height = 23
              DataField = 'RUA'
              DataSource = dsEndereco
              TabOrder = 2
            end
            object dbNumero: TDBEdit
              Left = 312
              Top = 55
              Width = 52
              Height = 23
              DataField = 'NUMERO'
              DataSource = dsEndereco
              TabOrder = 3
            end
            object DBEdit18: TDBEdit
              Left = 89
              Top = 15
              Width = 94
              Height = 23
              DataField = 'CEP'
              DataSource = dsEndereco
              MaxLength = 9
              TabOrder = 1
              OnExit = DBEdit18Exit
            end
            object dbLogradouro: TDBEdit
              Left = 366
              Top = 55
              Width = 308
              Height = 23
              DataField = 'LOGRADOURO'
              DataSource = dsEndereco
              TabOrder = 4
            end
            object dbBairro: TDBEdit
              Left = 2
              Top = 96
              Width = 338
              Height = 23
              DataField = 'BAIRRO'
              DataSource = dsEndereco
              TabOrder = 5
            end
            object dbCidade: TDBEdit
              Left = 343
              Top = 96
              Width = 331
              Height = 23
              DataField = 'CIDADE'
              DataSource = dsEndereco
              TabOrder = 6
            end
            object DBUF: TDBComboBox
              Left = 676
              Top = 96
              Width = 73
              Height = 23
              DataField = 'UF'
              DataSource = dm.Ds_endereco
              Items.Strings = (
                ''
                'AC'
                'AL'
                'AP'
                'AM'
                'BA'
                'CE'
                'ES'
                'GO'
                'MA'
                'MT'
                'MS'
                'MG'
                'PA'
                'PB'
                'PR'
                'PE'
                'PI'
                'RJ'
                'RN'
                'RS'
                'RO'
                'RR'
                'SC'
                'SP'
                'SE'
                'TO'
                'DF')
              TabOrder = 7
            end
            object DBGrid2: TDBGrid
              Left = 0
              Top = 123
              Width = 869
              Height = 98
              Align = alBottom
              DataSource = dsEndereco
              DrawingStyle = gdsGradient
              TabOrder = 8
              TitleFont.Charset = ANSI_CHARSET
              TitleFont.Color = clWindowText
              TitleFont.Height = -12
              TitleFont.Name = 'Segoe UI'
              TitleFont.Style = [fsBold]
              Columns = <
                item
                  Expanded = False
                  FieldName = 'ID'
                  Title.Caption = 'C'#243'digo'
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'RUA'
                  Title.Caption = 'Endere'#231'o'
                  Width = 254
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'NUMERO'
                  Title.Caption = 'N'#186
                  Width = 43
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'BAIRRO'
                  Title.Caption = 'Bairro'
                  Width = 292
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'CEP'
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'CIDADE'
                  Title.Caption = 'Cidade'
                  Width = 248
                  Visible = True
                end
                item
                  Expanded = False
                  FieldName = 'UF'
                  Visible = True
                end>
            end
          end
        end
      end
    end
  end
  inherited ImageList1: TImageList
    Left = 384
    Top = 8
    Bitmap = {
      
      000000000000}
  end
  inherited dsG: TDataSource
    DataSet = G
  end
  inherited G: TIBDataSet
    OnNewRecord = GNewRecord
    DeleteSQL.Strings = (
      'delete from CLIENTE'
      'where'
      '  ID = :OLD_ID')
    InsertSQL.Strings = (
      'insert into CLIENTE'
      
        '  (ID, NOME, CADASTRO, CPF, RG, DATA_NASCIMENTO, EMAIL, NOME_PAI' +
        ', NOME_MAE, '
      
        '   TIPO, ATIVO, EMAIL2, X, TELEFONE, TEL_RECADO, CELULAR, LIMITE' +
        'CREDITO, '
      '   IDUSUARIO)'
      'values'
      
        '  (:ID, :NOME, :CADASTRO, :CPF, :RG, :DATA_NASCIMENTO, :EMAIL, :' +
        'NOME_PAI, '
      
        '   :NOME_MAE, :TIPO, :ATIVO, :EMAIL2, :X, :TELEFONE, :TEL_RECADO' +
        ', :CELULAR, '
      '   :LIMITECREDITO, :IDUSUARIO)')
    RefreshSQL.Strings = (
      'Select '
      '  ID,'
      '  NOME,'
      '  CADASTRO,'
      '  CPF,'
      '  RG,'
      '  DATA_NASCIMENTO,'
      '  EMAIL,'
      '  NOME_PAI,'
      '  NOME_MAE,'
      '  TIPO,'
      '  ATIVO,'
      '  EMAIL2,'
      '  X,'
      '  TELEFONE,'
      '  TEL_RECADO,'
      '  CELULAR,'
      '  LIMITECREDITO,'
      '  IDUSUARIO'
      'from CLIENTE '
      'where'
      '  ID = :ID')
    SelectSQL.Strings = (
      'select  *  from CLIENTE'
      'where CLIENTE.X = 0'
      'order by id')
    ModifySQL.Strings = (
      'update CLIENTE'
      'set'
      '  ID = :ID,'
      '  NOME = :NOME,'
      '  CADASTRO = :CADASTRO,'
      '  CPF = :CPF,'
      '  RG = :RG,'
      '  DATA_NASCIMENTO = :DATA_NASCIMENTO,'
      '  EMAIL = :EMAIL,'
      '  NOME_PAI = :NOME_PAI,'
      '  NOME_MAE = :NOME_MAE,'
      '  TIPO = :TIPO,'
      '  ATIVO = :ATIVO,'
      '  EMAIL2 = :EMAIL2,'
      '  X = :X,'
      '  TELEFONE = :TELEFONE,'
      '  TEL_RECADO = :TEL_RECADO,'
      '  CELULAR = :CELULAR,'
      '  LIMITECREDITO = :LIMITECREDITO,'
      '  IDUSUARIO = :IDUSUARIO'
      'where'
      '  ID = :OLD_ID')
    GeneratorField.Field = 'ID'
    GeneratorField.Generator = 'GEN_CLIENTE_ID'
    object GID: TIntegerField
      FieldName = 'ID'
      Origin = '"CLIENTE"."ID"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object GNOME: TIBStringField
      FieldName = 'NOME'
      Origin = '"CLIENTE"."NOME"'
      Size = 100
    end
    object GCADASTRO: TDateField
      FieldName = 'CADASTRO'
      Origin = '"CLIENTE"."CADASTRO"'
      EditMask = '!99/99/0000;1;_'
    end
    object GCPF: TIBStringField
      FieldName = 'CPF'
      Origin = '"CLIENTE"."CPF"'
      EditMask = '000.000.000-00;1;_'
      Size = 14
    end
    object GRG: TIBStringField
      FieldName = 'RG'
      Origin = '"CLIENTE"."RG"'
      EditMask = '00.000.000-0;1;_'
      Size = 16
    end
    object GDATA_NASCIMENTO: TDateField
      FieldName = 'DATA_NASCIMENTO'
      Origin = '"CLIENTE"."DATA_NASCIMENTO"'
      EditMask = '!99/99/0000;1;_'
    end
    object GEMAIL: TIBStringField
      FieldName = 'EMAIL'
      Origin = '"CLIENTE"."EMAIL"'
      Size = 50
    end
    object GNOME_PAI: TIBStringField
      FieldName = 'NOME_PAI'
      Origin = '"CLIENTE"."NOME_PAI"'
      Size = 80
    end
    object GNOME_MAE: TIBStringField
      FieldName = 'NOME_MAE'
      Origin = '"CLIENTE"."NOME_MAE"'
      Size = 80
    end
    object GTIPO: TIBStringField
      FieldName = 'TIPO'
      Origin = '"CLIENTE"."TIPO"'
      FixedChar = True
      Size = 1
    end
    object GATIVO: TIBStringField
      FieldName = 'ATIVO'
      Origin = '"CLIENTE"."ATIVO"'
      FixedChar = True
      Size = 1
    end
    object GEMAIL2: TIBStringField
      FieldName = 'EMAIL2'
      Origin = '"CLIENTE"."EMAIL2"'
      Size = 60
    end
    object GX: TIBStringField
      FieldName = 'X'
      Origin = '"CLIENTE"."X"'
      FixedChar = True
      Size = 1
    end
    object GTELEFONE: TIBStringField
      FieldName = 'TELEFONE'
      Origin = '"CLIENTE"."TELEFONE"'
      EditMask = '!\(99\)0000-0000;1;_'
      Size = 18
    end
    object GTEL_RECADO: TIBStringField
      FieldName = 'TEL_RECADO'
      Origin = '"CLIENTE"."TEL_RECADO"'
      EditMask = '!\(99\)0000-0000;1;_'
      Size = 18
    end
    object GCELULAR: TIBStringField
      FieldName = 'CELULAR'
      Origin = '"CLIENTE"."CELULAR"'
      EditMask = '!\(99\)00000-0000;1;_'
      Size = 18
    end
    object GLIMITECREDITO: TIBBCDField
      FieldName = 'LIMITECREDITO'
      Origin = '"CLIENTE"."LIMITECREDITO"'
      DisplayFormat = '#0.00'
      Precision = 18
      Size = 2
    end
    object GIDUSUARIO: TIntegerField
      FieldName = 'IDUSUARIO'
      Origin = '"CLIENTE"."IDUSUARIO"'
    end
  end
  object dsEndereco: TDataSource
    DataSet = dm.ibEndCliente
    Left = 791
    Top = 350
  end
  object ACBrCEP: TACBrCEP
    ProxyPort = '8080'
    ParseText = True
    WebService = wsViaCep
    PesquisarIBGE = True
    Left = 467
    Top = 357
  end
  object ACBrValidador: TACBrValidador
    IgnorarChar = './-'
    AjustarTamanho = True
    PermiteVazio = True
    Left = 492
    Top = 113
  end
end

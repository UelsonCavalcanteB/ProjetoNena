inherited fmCadFuncionario: TfmCadFuncionario
  Caption = ''
  ClientHeight = 645
  ClientWidth = 1027
  ExplicitWidth = 1043
  ExplicitHeight = 683
  PixelsPerInch = 96
  TextHeight = 13
  inherited ToolBar1: TToolBar
    Width = 1027
    ExplicitWidth = 1027
    inherited btEditar: TToolButton
      OnClick = btEditarClick
    end
    inherited btExcluir: TToolButton
      OnClick = btExcluirClick
    end
  end
  inherited pnPesquisa: TPanel
    Width = 1027
    ExplicitWidth = 1027
    inherited btPesquisar: TSpeedButton
      OnClick = btPesquisarClick
    end
  end
  inherited PageControl1: TPageControl
    Width = 1027
    Height = 571
    ExplicitWidth = 1027
    ExplicitHeight = 571
    inherited tbDados: TTabSheet
      ExplicitWidth = 1019
      ExplicitHeight = 541
      inherited pnLegenda: TPanel
        Top = 512
        Width = 1019
        ExplicitTop = 512
        ExplicitWidth = 1019
      end
      inherited DBGrid1: TDBGrid
        Width = 1019
        Height = 512
        OnDrawColumnCell = DBGrid1DrawColumnCell
        Columns = <
          item
            Expanded = False
            FieldName = 'ID'
            Title.Caption = 'C'#243'digo'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME'
            Title.Caption = 'Nome'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CADASTRO'
            Title.Caption = 'Cadastro'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CPF'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RG'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DATA_NASCIMENTO'
            Title.Caption = 'Nascimento'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ATIVO'
            Title.Caption = 'Ativo'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CARTEIRA_NUMERO'
            Title.Caption = 'Cta. N'#250'mero'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CARTEIRA_SERIE'
            Title.Caption = 'Cta. S'#233'rie'
            Visible = True
          end>
      end
    end
    inherited tbCadastro: TTabSheet
      ExplicitWidth = 1019
      ExplicitHeight = 541
      object Label3: TLabel
        Left = 3
        Top = 9
        Width = 38
        Height = 15
        Caption = 'C'#243'digo'
        FocusControl = DBEdit1
      end
      object Label4: TLabel
        Left = 65
        Top = 9
        Width = 48
        Height = 15
        Caption = 'Cadastro'
        FocusControl = DBEdit2
      end
      object Label5: TLabel
        Left = 3
        Top = 51
        Width = 34
        Height = 15
        Caption = 'Nome'
        FocusControl = DBEdit3
      end
      object Label6: TLabel
        Left = 428
        Top = 51
        Width = 20
        Height = 15
        Caption = 'CPF'
        FocusControl = DBEdit4
      end
      object Label7: TLabel
        Left = 531
        Top = 51
        Width = 17
        Height = 15
        Caption = 'RG'
        FocusControl = DBEdit5
      end
      object Label8: TLabel
        Left = 3
        Top = 96
        Width = 24
        Height = 15
        Caption = 'M'#227'e'
        FocusControl = DBEdit6
      end
      object Label9: TLabel
        Left = 634
        Top = 51
        Width = 50
        Height = 15
        Caption = 'Telefone'
        FocusControl = DBEdit7
      end
      object Label10: TLabel
        Left = 745
        Top = 51
        Width = 38
        Height = 15
        Caption = 'Celular'
        FocusControl = DBEdit8
      end
      object Label11: TLabel
        Left = 427
        Top = 96
        Width = 16
        Height = 15
        Caption = 'Pai'
        FocusControl = DBEdit9
      end
      object Label12: TLabel
        Left = 481
        Top = 135
        Width = 75
        Height = 15
        Caption = 'Registro CNH'
        FocusControl = DBEdit10
      end
      object Label13: TLabel
        Left = 568
        Top = 135
        Width = 54
        Height = 15
        Caption = 'Data CNH'
        FocusControl = DBEdit11
      end
      object Label14: TLabel
        Left = 661
        Top = 134
        Width = 75
        Height = 15
        Caption = 'Validade CNH'
        FocusControl = DBEdit12
      end
      object Label19: TLabel
        Left = 214
        Top = 9
        Width = 39
        Height = 15
        Caption = 'Inativo'
        FocusControl = DBEdit13
        Visible = False
      end
      object Label20: TLabel
        Left = 3
        Top = 181
        Width = 96
        Height = 15
        Caption = 'Cta. Identifica'#231#227'o'
        FocusControl = DBEdit14
      end
      object Label21: TLabel
        Left = 128
        Top = 181
        Width = 70
        Height = 15
        Caption = 'Cta. N'#250'mero'
        FocusControl = DBEdit15
      end
      object Label22: TLabel
        Left = 224
        Top = 181
        Width = 53
        Height = 15
        Caption = 'Cta. S'#233'rie'
        FocusControl = DBEdit16
      end
      object Label23: TLabel
        Left = 296
        Top = 181
        Width = 39
        Height = 15
        Caption = 'Cta. UF'
        FocusControl = DBEdit17
      end
      object Label27: TLabel
        Left = 3
        Top = 135
        Width = 34
        Height = 15
        Caption = 'E-mail'
        FocusControl = DBEdit20
      end
      object Label28: TLabel
        Left = 427
        Top = 136
        Width = 48
        Height = 15
        Caption = 'Tipo Cnh'
      end
      object DBEdit1: TDBEdit
        Left = 3
        Top = 25
        Width = 61
        Height = 23
        DataField = 'ID'
        DataSource = dsG
        TabOrder = 0
      end
      object DBEdit2: TDBEdit
        Left = 65
        Top = 25
        Width = 91
        Height = 23
        DataField = 'CADASTRO'
        DataSource = dsG
        TabOrder = 1
      end
      object DBCheckBox1: TDBCheckBox
        Left = 162
        Top = 28
        Width = 97
        Height = 17
        Caption = 'Ativo'
        DataField = 'ATIVO'
        DataSource = dsG
        TabOrder = 2
        ValueChecked = 'S'
        ValueUnchecked = 'N'
      end
      object DBEdit3: TDBEdit
        Left = 3
        Top = 67
        Width = 422
        Height = 23
        DataField = 'NOME'
        DataSource = dsG
        TabOrder = 3
      end
      object DBEdit4: TDBEdit
        Left = 428
        Top = 67
        Width = 101
        Height = 23
        DataField = 'CPF'
        DataSource = dsG
        TabOrder = 4
      end
      object DBEdit5: TDBEdit
        Left = 531
        Top = 67
        Width = 101
        Height = 23
        DataField = 'RG'
        DataSource = dsG
        TabOrder = 5
      end
      object DBEdit6: TDBEdit
        Left = 3
        Top = 111
        Width = 422
        Height = 23
        DataField = 'NOME_MAE'
        DataSource = dsG
        TabOrder = 8
      end
      object DBEdit7: TDBEdit
        Left = 634
        Top = 67
        Width = 109
        Height = 23
        DataField = 'TELEFONE'
        DataSource = dsG
        TabOrder = 6
      end
      object DBEdit8: TDBEdit
        Left = 745
        Top = 67
        Width = 100
        Height = 23
        DataField = 'CELULAR'
        DataSource = dsG
        TabOrder = 7
      end
      object DBEdit9: TDBEdit
        Left = 427
        Top = 111
        Width = 418
        Height = 23
        DataField = 'NOME_PAI'
        DataSource = dsG
        TabOrder = 9
      end
      object DBEdit10: TDBEdit
        Left = 481
        Top = 151
        Width = 84
        Height = 23
        DataField = 'REGISTRO_CNH'
        DataSource = dsG
        TabOrder = 12
      end
      object DBEdit11: TDBEdit
        Left = 568
        Top = 151
        Width = 91
        Height = 23
        DataField = 'DATA_CNH'
        DataSource = dsG
        TabOrder = 13
      end
      object DBEdit12: TDBEdit
        Left = 661
        Top = 150
        Width = 91
        Height = 23
        DataField = 'DATAVALIDADE_CNH'
        DataSource = dsG
        TabOrder = 14
      end
      object DBEdit13: TDBEdit
        Left = 214
        Top = 25
        Width = 83
        Height = 23
        DataField = 'DATA_INATIVADO'
        DataSource = dsG
        TabOrder = 19
        Visible = False
      end
      object DBEdit14: TDBEdit
        Left = 3
        Top = 195
        Width = 119
        Height = 23
        DataField = 'CARTEIRA_IDENTIFICACAO'
        DataSource = dsG
        TabOrder = 15
      end
      object DBEdit15: TDBEdit
        Left = 128
        Top = 196
        Width = 95
        Height = 23
        DataField = 'CARTEIRA_NUMERO'
        DataSource = dsG
        TabOrder = 16
      end
      object DBEdit16: TDBEdit
        Left = 224
        Top = 196
        Width = 71
        Height = 23
        DataField = 'CARTEIRA_SERIE'
        DataSource = dsG
        TabOrder = 17
      end
      object DBEdit17: TDBEdit
        Left = 296
        Top = 196
        Width = 39
        Height = 23
        DataField = 'CARTEIRA_UF'
        DataSource = dsG
        TabOrder = 18
      end
      object Panel1: TPanel
        Left = 341
        Top = 180
        Width = 374
        Height = 52
        BevelOuter = bvNone
        TabOrder = 20
        object Label24: TLabel
          Left = 0
          Top = 1
          Width = 42
          Height = 15
          Caption = 'Usu'#225'rio'
          FocusControl = DBEdit18
        end
        object Label25: TLabel
          Left = 115
          Top = 1
          Width = 34
          Height = 15
          Caption = 'Senha'
          FocusControl = DBEdit19
        end
        object Label26: TLabel
          Left = 234
          Top = 0
          Width = 80
          Height = 15
          Caption = 'Repita a senha'
        end
        object DBEdit18: TDBEdit
          Left = 0
          Top = 17
          Width = 113
          Height = 23
          DataField = 'USUARIO'
          DataSource = dsG
          TabOrder = 0
        end
        object DBEdit19: TDBEdit
          Left = 115
          Top = 17
          Width = 117
          Height = 23
          DataField = 'SENHA'
          DataSource = dsG
          PasswordChar = '*'
          TabOrder = 1
        end
        object Edit3: TEdit
          Left = 234
          Top = 17
          Width = 121
          Height = 23
          PasswordChar = '*'
          TabOrder = 2
        end
      end
      object DBEdit20: TDBEdit
        Left = 3
        Top = 150
        Width = 422
        Height = 23
        DataField = 'EMAIL'
        DataSource = dsG
        TabOrder = 10
      end
      object Panel2: TPanel
        Left = 0
        Top = 224
        Width = 846
        Height = 236
        BevelOuter = bvNone
        Caption = 'Panel2'
        TabOrder = 21
        inline frmEndereco1: TfrmEndereco
          Left = 0
          Top = 0
          Width = 846
          Height = 236
          Align = alClient
          TabOrder = 0
          ExplicitWidth = 846
          inherited PageControl1: TPageControl
            Width = 846
            ExplicitWidth = 846
            inherited TabSheet1: TTabSheet
              ExplicitLeft = 4
              ExplicitTop = 28
              ExplicitWidth = 838
              ExplicitHeight = 204
              inherited Panel1: TPanel
                Width = 838
                ExplicitWidth = 838
                inherited DBGrid1: TDBGrid
                  Width = 836
                  Columns = <
                    item
                      Expanded = False
                      FieldName = 'ID'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Caption = 'C'#243'dgo'
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Width = 64
                      Visible = True
                    end
                    item
                      Expanded = False
                      FieldName = 'RUA'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Caption = 'Endere'#231'o'
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Width = 108
                      Visible = True
                    end
                    item
                      Expanded = False
                      FieldName = 'NUMERO'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Caption = 'N'#186
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Width = 59
                      Visible = True
                    end
                    item
                      Expanded = False
                      FieldName = 'CEP'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Caption = 'Cep'
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Visible = True
                    end
                    item
                      Expanded = False
                      FieldName = 'LOGRADOURO'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Caption = 'Logradouro'
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Width = 105
                      Visible = True
                    end
                    item
                      Expanded = False
                      FieldName = 'BAIRRO'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Caption = 'Bairro'
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Visible = True
                    end
                    item
                      Expanded = False
                      FieldName = 'CIDADE'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Caption = 'Cidade'
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Visible = True
                    end
                    item
                      Expanded = False
                      FieldName = 'UF'
                      Font.Charset = ANSI_CHARSET
                      Font.Color = clWindowText
                      Font.Height = -12
                      Font.Name = 'Segoe UI'
                      Font.Style = [fsBold]
                      Title.Font.Charset = ANSI_CHARSET
                      Title.Font.Color = clWindowText
                      Title.Font.Height = -13
                      Title.Font.Name = 'Arial'
                      Title.Font.Style = [fsBold]
                      Visible = True
                    end>
                end
              end
            end
          end
        end
      end
      object DBComboBox1: TDBComboBox
        Left = 428
        Top = 150
        Width = 51
        Height = 23
        DataField = 'CNH_TIPO'
        DataSource = dsG
        Items.Strings = (
          'A'
          'AB'
          'B'
          'C'
          'D'
          'E')
        TabOrder = 11
      end
    end
    object TabSheet1: TTabSheet
      Caption = 'Permiss'#227'o'
      ImageIndex = 2
    end
  end
  inherited ImageList1: TImageList
    Left = 840
    Top = 24
    Bitmap = {
      
      000000000000}
  end
  inherited dsG: TDataSource
    DataSet = G
  end
  inherited G: TIBDataSet
    DeleteSQL.Strings = (
      'delete from FUNCIONARIO'
      'where'
      '  ID = :OLD_ID')
    InsertSQL.Strings = (
      'insert into FUNCIONARIO'
      
        '  (ID, NOME, CPF, RG, DATA_NASCIMENTO, TELEFONE, CELULAR, NOME_M' +
        'AE, NOME_PAI, '
      
        '   CNH_TIPO, REGISTRO_CNH, DATA_CNH, DATAVALIDADE_CNH, CADASTRO,' +
        ' DATA_INATIVADO, '
      
        '   CARTEIRA_IDENTIFICACAO, CARTEIRA_NUMERO, CARTEIRA_SERIE, CART' +
        'EIRA_UF, '
      '   USUARIO, SENHA, ATIVO, EMAIL, EX, IDUSUARIO)'
      'values'
      
        '  (:ID, :NOME, :CPF, :RG, :DATA_NASCIMENTO, :TELEFONE, :CELULAR,' +
        ' :NOME_MAE, '
      
        '   :NOME_PAI, :CNH_TIPO, :REGISTRO_CNH, :DATA_CNH, :DATAVALIDADE' +
        '_CNH, :CADASTRO, '
      
        '   :DATA_INATIVADO, :CARTEIRA_IDENTIFICACAO, :CARTEIRA_NUMERO, :' +
        'CARTEIRA_SERIE, '
      
        '   :CARTEIRA_UF, :USUARIO, :SENHA, :ATIVO, :EMAIL, :EX, :IDUSUAR' +
        'IO)')
    RefreshSQL.Strings = (
      'Select '
      '  ID,'
      '  NOME,'
      '  CPF,'
      '  RG,'
      '  DATA_NASCIMENTO,'
      '  TELEFONE,'
      '  CELULAR,'
      '  NOME_MAE,'
      '  NOME_PAI,'
      '  CNH_TIPO,'
      '  REGISTRO_CNH,'
      '  DATA_CNH,'
      '  DATAVALIDADE_CNH,'
      '  CADASTRO,'
      '  DATA_INATIVADO,'
      '  CARTEIRA_IDENTIFICACAO,'
      '  CARTEIRA_NUMERO,'
      '  CARTEIRA_SERIE,'
      '  CARTEIRA_UF,'
      '  USUARIO,'
      '  SENHA,'
      '  ATIVO,'
      '  EMAIL,'
      '  EX,'
      '  IDUSUARIO'
      'from FUNCIONARIO '
      'where'
      '  ID = :ID')
    SelectSQL.Strings = (
      'select *  from  FUNCIONARIO'
      'where EX = 0')
    ModifySQL.Strings = (
      'update FUNCIONARIO'
      'set'
      '  ID = :ID,'
      '  NOME = :NOME,'
      '  CPF = :CPF,'
      '  RG = :RG,'
      '  DATA_NASCIMENTO = :DATA_NASCIMENTO,'
      '  TELEFONE = :TELEFONE,'
      '  CELULAR = :CELULAR,'
      '  NOME_MAE = :NOME_MAE,'
      '  NOME_PAI = :NOME_PAI,'
      '  CNH_TIPO = :CNH_TIPO,'
      '  REGISTRO_CNH = :REGISTRO_CNH,'
      '  DATA_CNH = :DATA_CNH,'
      '  DATAVALIDADE_CNH = :DATAVALIDADE_CNH,'
      '  CADASTRO = :CADASTRO,'
      '  DATA_INATIVADO = :DATA_INATIVADO,'
      '  CARTEIRA_IDENTIFICACAO = :CARTEIRA_IDENTIFICACAO,'
      '  CARTEIRA_NUMERO = :CARTEIRA_NUMERO,'
      '  CARTEIRA_SERIE = :CARTEIRA_SERIE,'
      '  CARTEIRA_UF = :CARTEIRA_UF,'
      '  USUARIO = :USUARIO,'
      '  SENHA = :SENHA,'
      '  ATIVO = :ATIVO,'
      '  EMAIL = :EMAIL,'
      '  EX = :EX,'
      '  IDUSUARIO = :IDUSUARIO'
      'where'
      '  ID = :OLD_ID')
    object GID: TIntegerField
      FieldName = 'ID'
      Origin = '"FUNCIONARIO"."ID"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object GNOME: TIBStringField
      FieldName = 'NOME'
      Origin = '"FUNCIONARIO"."NOME"'
      Size = 80
    end
    object GCPF: TIBStringField
      FieldName = 'CPF'
      Origin = '"FUNCIONARIO"."CPF"'
      Size = 14
    end
    object GRG: TIBStringField
      FieldName = 'RG'
      Origin = '"FUNCIONARIO"."RG"'
      Size = 14
    end
    object GDATA_NASCIMENTO: TDateField
      FieldName = 'DATA_NASCIMENTO'
      Origin = '"FUNCIONARIO"."DATA_NASCIMENTO"'
    end
    object GTELEFONE: TIBStringField
      FieldName = 'TELEFONE'
      Origin = '"FUNCIONARIO"."TELEFONE"'
      Size = 10
    end
    object GCELULAR: TIBStringField
      FieldName = 'CELULAR'
      Origin = '"FUNCIONARIO"."CELULAR"'
      Size = 12
    end
    object GNOME_MAE: TIBStringField
      FieldName = 'NOME_MAE'
      Origin = '"FUNCIONARIO"."NOME_MAE"'
      Size = 80
    end
    object GNOME_PAI: TIBStringField
      FieldName = 'NOME_PAI'
      Origin = '"FUNCIONARIO"."NOME_PAI"'
      Size = 80
    end
    object GCNH_TIPO: TIBStringField
      FieldName = 'CNH_TIPO'
      Origin = '"FUNCIONARIO"."CNH_TIPO"'
      FixedChar = True
      Size = 1
    end
    object GREGISTRO_CNH: TIBStringField
      FieldName = 'REGISTRO_CNH'
      Origin = '"FUNCIONARIO"."REGISTRO_CNH"'
      Size = 12
    end
    object GDATA_CNH: TDateField
      FieldName = 'DATA_CNH'
      Origin = '"FUNCIONARIO"."DATA_CNH"'
    end
    object GDATAVALIDADE_CNH: TDateField
      FieldName = 'DATAVALIDADE_CNH'
      Origin = '"FUNCIONARIO"."DATAVALIDADE_CNH"'
    end
    object GCADASTRO: TDateField
      FieldName = 'CADASTRO'
      Origin = '"FUNCIONARIO"."CADASTRO"'
    end
    object GDATA_INATIVADO: TDateField
      FieldName = 'DATA_INATIVADO'
      Origin = '"FUNCIONARIO"."DATA_INATIVADO"'
    end
    object GCARTEIRA_IDENTIFICACAO: TIntegerField
      FieldName = 'CARTEIRA_IDENTIFICACAO'
      Origin = '"FUNCIONARIO"."CARTEIRA_IDENTIFICACAO"'
    end
    object GCARTEIRA_NUMERO: TIntegerField
      FieldName = 'CARTEIRA_NUMERO'
      Origin = '"FUNCIONARIO"."CARTEIRA_NUMERO"'
    end
    object GCARTEIRA_SERIE: TIntegerField
      FieldName = 'CARTEIRA_SERIE'
      Origin = '"FUNCIONARIO"."CARTEIRA_SERIE"'
    end
    object GCARTEIRA_UF: TIBStringField
      FieldName = 'CARTEIRA_UF'
      Origin = '"FUNCIONARIO"."CARTEIRA_UF"'
      Size = 2
    end
    object GUSUARIO: TIBStringField
      FieldName = 'USUARIO'
      Origin = '"FUNCIONARIO"."USUARIO"'
    end
    object GSENHA: TIntegerField
      FieldName = 'SENHA'
      Origin = '"FUNCIONARIO"."SENHA"'
    end
    object GATIVO: TIBStringField
      FieldName = 'ATIVO'
      Origin = '"FUNCIONARIO"."ATIVO"'
      FixedChar = True
      Size = 1
    end
    object GEMAIL: TIBStringField
      FieldName = 'EMAIL'
      Origin = '"FUNCIONARIO"."EMAIL"'
      Size = 60
    end
    object GEX: TIBStringField
      FieldName = 'EX'
      Origin = '"FUNCIONARIO"."EX"'
      FixedChar = True
      Size = 1
    end
    object GIDUSUARIO: TIntegerField
      FieldName = 'IDUSUARIO'
      Origin = '"FUNCIONARIO"."IDUSUARIO"'
    end
  end
end
